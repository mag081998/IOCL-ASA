...
import in.cdac.aadhaarservices.daooperations.bfd.BfdDaoOperations;
...
public final class BfdRequestProcessor extends RequestProcessor
{
	private static final Logger logger = LogManager.getLogger(AuthRequestProcessor.class);

	@Autowired
	private BfdDaoOperations authDaoOperations;
	...
	public String processReq(String authReqXML, String clientIP,String lk) 
	{
		...
		try
		{
			...
			Map<String, String> checkDuplicateTxn=authDaoOperations.insertTxnAtStart(authReqObj.getTxn(), "BFD");
			...
			if (signrequired) 
			{
				...
				try {
					...
					{
						CryptoACLKCert crypaclkcert = authDaoOperations.getCryptoAcLkCertDet(txnRecord.getAsaAc(),
				...
				{
					logger.info("Error in Parsing the modified Auth XML in Class Name:BfdRequestProcessor Method Name: processReq",excp);
					return errorResponse( "UNABLE_AUTH_XML_PARSING", false, txnRecord ,authReqObj);
				}
			} 
			else 
			...
			url = generateURL( uidaiBfdUrl, uidCharArray, lkDet.getAsa_license_key(), authReqObj.getAc() );
		...
		{
			logger.info("Main Exception in Class Name:BfdRequestProcessor Method Name: processReq",excp);
	...
	private Map<String, String> digitalSigning( AuthTransactionRecord txnRecord, String authReqXml, CryptoACLKCert certdet )
	{
		...
		{
			logger.info("Crypto Service Invocation or Marshalling error in Class Name:BfdRequestProcessor Method Name: digitalSigning",excp);
			mapResult = getMapResult("n", "AUTH_SIGNING_ISSUE");
			return mapResult;
		}
		return mapResult;
	...
	private String errorResponse( String errMsg, boolean logInDB, AuthTransactionRecord txnRecord,Auth auth ) 
	{
		...
		logger.info("Error returned in Class Name:BfdRequestProcessor Method Name: errorResponse:"+ "Txn:" +txnRecord.getTransactionId()+ "ErrorCode:" +errorCode + "ErrorMessage:" +errMsg);
		return authResponse;
	}
	/**
	 * 
	...
	private String updateResponse( String responseXml, AuthTransactionRecord txnRecord ,Auth authReqObj)
	{
		...
		{
			logger.info("Error in Class Name:BfdRequestProcessor Method Name: updateResponse in parsing the response XML",excp);
