...
@Component
@EnableAsync
public  class BfdDaoOperations extends DaoCommon 
{
	private static final Logger logger = LogManager.getLogger(BfdDaoOperations.class);
	private static final Logger bfddbinfile = LogManager.getLogger("bfddbinfile");
...
public static String ipAddress = "0.0.0.0";
	...
	public void insertASAAuthRecord(Auth auth,AuthTransactionRecord transactionRecord) 
	{
		...
		try
		{
			String sql = BfdDBQueries.insertASAAuthQuery(schemaName);
	...
	private void insertInFile( Auth auth,AuthTransactionRecord aTxn, Map<String, String> map ) 
	{
		...
		if ( ((String)map.get(Constants.STATUS)).equalsIgnoreCase(Constants.NO) )
		{
			...
			try 
			{
				...
				bfddbinfile.info(strbld.toString());
			}
			catch(Exception excp)
			{
